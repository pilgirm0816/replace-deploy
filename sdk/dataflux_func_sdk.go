...
// PrettyPrint Beautify console printing
func PrettyPrint(v interface{}) interface{} {
	b, err := json.Marshal(v)
	if err != nil {
		fmt.Println(v)
		return nil
	}
	var outDta bytes.Buffer
	err = json.Indent(&outDta, b, "", "  ")
	if err != nil {
		fmt.Println(v)
		return nil
	}
	return outDta.String()
}
...
func (dff *DataFluxFunc) run(method, path string, query url.Values, body interface{}, header http.Header, traceID string) (int, interface{}, error) {
	...
	if dff.UseHttps {
		...
		reqUrl = "https://" + dff.Host + ":" + dff.Port + path
	} else {
		transport = http.DefaultTransport
		reqUrl = "http://" + dff.Host + ":" + dff.Port + path
	}
...
// Upload files to the file manager in Func
...
// Post request
func (dff *DataFluxFunc) Post(path string, query url.Values, body interface{}, headers http.Header, traceID string) (int, interface{}, error) {
	return dff.run("POST", path, query, body, headers, traceID)
}

// Put request
func (dff *DataFluxFunc) Put(path string, query url.Values, body interface{}, headers http.Header, traceID string) (int, interface{}, error) {
