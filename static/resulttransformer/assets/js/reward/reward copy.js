$(()=>{
  ...
  getPerson = (personlist) => {
    ...
    addscore.on('click', ()=>{
      ...
      trcontainer += `<td><select onchange="clickName(this)" class="${person_key_array[0]}_${person_idx} form-control form-control-sm" item="${person_idx}">${namehtml}</select></td>`
  ...
  getProject = (projectlist) => {
    let project_key_array = ['name', 'po_num', 'contract_num', 'totalmoney','cost','paymoney','recievemoney','totalaward','saleaward','award','poolmoney','restaward']
    ...
    projectspromise.then((rs)=>{
      ...
      $('#addproject').on('click', ()=>{
        ...
        for(let i = 1;i < ths.length-1;i++) {
          let attr = ''
          let tdattr = ''
          if (project_key_array[i] == 'totalaward' || project_key_array[i] == 'restaward' || project_key_array[i] == 'totalmoney') {
            attr = 'readonly=readonly'
          } else if (project_key_array[i] == 'po_num' || project_key_array[i] == 'contract_num') {
      ...
      changeProjectName = (idx, val) => {
        ...
        $(`.recievemoney_${idx}`).on('blur', (e)=>{
          ...
          let _saleaward = _recieve * 0.02
          let _award = _recieve * 0.045
          let _pool = _recieve * 0.055
          $(`.saleaward_${idx}`).val(_saleaward.toFixed(3))
          $(`.award_${idx}`).val(_award.toFixed(3))
          $(`.poolmoney_${idx}`).val(_pool.toFixed(3))
          // 触发 项目奖金计算
        ...
        project_key_array.forEach((key)=>{
          ...
          // } else 
          if ($.inArray(key, ['cost','paymoney','recievemoney','saleaward','award']) != -1) {
            $(`.${key}_${idx}`).val(0)
          } else if (key == 'restaward') {
            // 剩余奖金归零了，就不能在分配了。
            let _rest = +(obj['totalaward'] - obj['award']).toFixed(3)
            if (_rest <= 0) {
              $(`.award_${idx}`).attr('readonly','readonly')
              $(`.${key}_${idx}`).val(Math.abs(_rest).toFixed(3))
            } else {
              $(`.award_${idx}`).removeAttr('readonly')
              $(`.${key}_${idx}`).val((_rest).toFixed(3))
            }
          } else if (key == 'totalmoney') {
            $(`.${key}_${idx}`).val(`${(obj['totalmoney']).toFixed(3)}/${(obj['totalmoneybeforetax']).toFixed(3)}`)
          } else {
  ...
  if (awardid) {
    ...
    if (typestr == 'edit') {
      ...
      awardlistpromise.then((item)=>{
        ...
        $('#poolaward').val(poolaward.toFixed(3))
        $('#totalpool').val(totalpool.toFixed(3))
        // 人员关系数据
        ...
        personlist.forEach((person, person_idx)=>{
          ...
          let trcontainer = `<tr class="list" idx="${person_idx}">`
          // trcontainer += `<td><select onchange="clickName(this)" class="${person_key_array[0]}_${person_idx} form-control form-control-sm" item="${person_idx}">${namehtml}</select></td>`
          trcontainer += `<td><select></select>${person[person_detail_key_array[0]]}</td>`
          for(let i=1;i<ths.length-1;i++) {
        ...
        projectlist.forEach((project, project_idx)=>{
          ...
          for(let i = 0;i < ths.length-1;i++) {
            if (Array.isArray(project_detail_key_array[i])) {
              ...
              if ($.inArray(project_detail_key_array[i], ['po_num','contract_num']) != -1) {
                tdattr = 'style="display:none;"'
              } else if (project_detail_key_array[i] == 'restaward') {
                project_val = Math.abs(project['totalaward'] - project['sumaward']).toFixed(3)
              }
        ...
        getProject(projectlist)
        // 累计销售奖金
        $('#saleaward').val(totalsaleaward.toFixed(3))
        // 累计进池子总奖金
        $('#poolmoney').val(totalpoolmoney.toFixed(3))
        // 累计 totalaward
        $('#award').val(totalaward.toFixed(3))
      ...
      awardlistpromise.then((item)=>{
        ...
        $('#poolmoney').val(totalpoolmoney.toFixed(3))
        // 累计 totalaward
        $('#award').val(totalaward.toFixed(3)).attr('readonly','readonly')
    ...
    let awardlistpromise = getPromise(api['添加录入记录'])
  ...
  calcamount = (index) => {
    ...
    rate = trouble * troublerate * 0.01 + duration * durationrate * 0.01 + response * responserate * 0.01
    ...
    for (let i = 0; i < $('#score tbody tr.list').length; i++) {
      ...
      $(`.percent_${_idx}`).val((+_percent*100).toFixed(2))
      $(`.amount_${_idx}`).val(_amount.toFixed(2))
    }
  }
  ...
  $(document).on('blur', '#projects input', (d)=>{
    ...
    for (let i = 0; i < $('#projects tbody tr.list').length; i++) {
      ...
      award += +$(`.award_${_idx}`).val() * (1 - 0.083)
      saleaward += +$(`.saleaward_${_idx}`).val()
      poolmoney += +$(`.poolmoney_${_idx}`).val()
    }
    $('#saleaward').val(saleaward.toFixed(3))
    $('#poolmoney').val(poolmoney.toFixed(3))
    $('#award').val(award.toFixed(3)).trigger('change')
  })
  ...
  $('#submit').on('click', (e)=>{
    // 添加时间表，生成 award_id
    let addawardpromise = getPromise(api['添加录入时间']);
