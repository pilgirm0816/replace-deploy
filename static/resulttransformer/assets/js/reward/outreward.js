$(()=>{
      ...
      doAddscore = (personObj, idx) => {
        ...
        if (personObj) {
          ...
          for(let i=1;i<ths.length-2;i++) {
            let attr = ''
            if($.inArray(person_key_array[i],['trouble','duration','response','rate','percent','saleamount','poolamount']) != -1) {
              attr = 'style="display:none";'
            }
            trcontainer += `<td ${attr}><input class="${person_key_array[i]}_${person_idx} form-control form-control-sm" value="${personObj[contract_person_key_array[person_key_array[i]]]}"></td>`
          }
        } else {
          ...
          for(let i=1;i<ths.length-2;i++) {
            ...
            trcontainer += `<td ${attr}><input class="${person_key_array[i]}_${person_idx} form-control form-control-sm" value="0"></td>`
          }
        }
  ...
  getProject = (projectlist) => {
    let project_key_array = ['name', 'isdirect', 'po_num', 'contract_num', 'totalmoney','cost','paymoney','paymoneytax','recievemoney','totalaward','saleaward','award','poolmoney','restaward']
    ...
    projectspromise.then((rs)=>{
      ...
      doAddproject = (projectObj, idx) => {
        ...
        if (projectObj) {
          ...
          for(let i = 1;i < ths.length-1;i++) {
            ...
            } else if (['po_num','contract_num','cost','recievemoney','saleaward','poolmoney','paymoneytax'].includes(project_key_array[i])) {
      ...
      changeProjectName = (idx, val) => {
        ...
        project_key_array.forEach((key)=>{
          ...
          if (['cost','paymoney','recievemoney','saleaward','award','paymoneytax'].includes(key)) {
            $(`.${key}_${idx}`).val(0)
  ...
  if (awardid) {
    ...
    // 总项目奖金、总销售奖金、总奖金池金额
    let totalaward = 0,totalsaleaward = 0,totalpoolmoney = 0, totalpaymoney = 0;
    // 成员字段
    ...
    if (typestr == 'edit') {
      ...
      awardlistpromise.then((item)=>{
        ...
        projectlist.forEach((project, project_idx)=>{
          ...
          totalpaymoney += +project['paymoney']
        })
        ...
        // 支出供应商含税
        $('#totalpaymoneyaddtax').val((toFixed(totalpaymoney,3))).attr('readonly','readonly')
        // 支出供应商税费
        $('#totalpaymoneytax').val(toFixed((totalpaymoney - totalpaymoney / (1 + 0.08)),3)).attr('readonly','readonly')
        // 累计奖金含税
        $('#awardaddtax').val(toFixed($('#award').val() * (1+0.083),3))
        // 奖金的税
      ...
      awardlistpromise.then((item)=>{
        ...
        personlist.forEach((person, person_idx)=>{
          ...
          for(let i=0;i<ths.length-1;i++) {
            ...
            } else if($.inArray(person_detail_key_array[i],['troublerate','durationrate','responserate','ratio','percent','saleamount','poolamount']) != -1) {
              trcontainer += `<td style="display:none">${person[person_detail_key_array[i]]}</td>`
            } else {
              trcontainer += `<td>${person[person_detail_key_array[i]]}</td>`
            }
          }
        ...
        projectlist.forEach((project, project_idx)=>{
          ...
          for(let i = 0;i < ths.length-2;i++) {
            if (Array.isArray(project_detail_key_array[i])) {
              ...
              if (['po_num','contract_num','cost','recievemoney','saleaward','poolmoney'].includes(project_detail_key_array[i])) {
                tdattr = 'style="display:none;"'
        ...
        $('#totalpaymoneyaddtax').val((toFixed(totalpaymoney,3))).attr('readonly','readonly')
        // 支出供应商税费
        $('#totalpaymoneytax').val(toFixed((totalpaymoney - totalpaymoney / (1 + 0.08)),3))
        // 累计奖金含税
        $('#awardaddtax').val(toFixed($('#award').val() * (1+0.083),3))
        // 奖金的税
        $('#saleawardtax').val((totalsaleaward - toFixed(totalsaleaward / (1 + 0.083),3)).toFixed(3))
    ...
    let awardlistpromise = getPromise(`${api['添加录入记录']}?params=2`)
    awardlistpromise.then((rs)=>{
      ...
      awardlist.on('click', (e)=>{
        location.href=`./outreward.html?params=${$(e.currentTarget).attr('awardid')}`
      })
    })
  }
  ...
  $('#toawardlist').on('click', ()=>{
    location.href=`./outreward.html`
    // history.go(-1)
  })

  // 编辑按钮
  $('#editaward').on('click', ()=>{
    location.href=`./outreward.html?params=${awardid}&type=edit`
  })
  ...
  calcamount = (index) => {
    ...
    for (let i = 0; i < $('#score tbody tr.list').length; i++) {
      ...
      let _percent = ratesum ? +_rate/+ratesum : 0
  ...
  $(document).on('blur', '#projects input', (d)=>{
    // 项目奖金, 销售奖金, 池子金额
    let award = 0, saleaward = 0, poolmoney = 0, paymoney = 0
    for (let i = 0; i < $('#projects tbody tr.list').length; i++) {
      ...
      let _paymoney = $(`.paymoney_${_idx}`).val()
      $(`.paymoneytax_${_idx}`).val(toFixed(_paymoney * (1 + 0.08),3))
    }
    // 销售
    ...
    $('#award').val(toFixed(award,3)).trigger('change')
    // 累计奖金含税
    $('#awardaddtax').val(toFixed($('#award').val() * (1+0.083),3))
    // 奖金的税
    $('#saleawardtax').val((saleaward - toFixed(saleaward / (1 + 0.083),3)).toFixed(3))
    // 销售的税
    $('#awardtax').val((toFixed((award * 0.083),3)))

    // 供应商
    $('#totalpaymoney').val((toFixed(paymoney / (1 + 0.08),3)))
    // 供应商含税
    $('#totalpaymoneyaddtax').val((toFixed(paymoney,3)))
    // 供应商税
    $('#totalpaymoneytax').val(toFixed((paymoney - paymoney / (1 + 0.08)),3))
  })
    ...
    doAddaward = (award_id, isEdit) => {
      ...
      for (let i = 0; i < $('#projects tbody tr.list').length; i++) {
        ...
        let _item = {
          ...
          cost: +$(`.cost_${_idx}`).val(),
          paymoney: +$(`.paymoney_${_idx}`).val(),
          poolmoney: 0, // +$(`.poolmoney_${_idx}`).val(),
          saleaward: 0, // +$(`.saleaward_${_idx}`).val(),
          recievemoney: +$(`.award_${_idx}`).val(), // +$(`.recievemoney_${_idx}`).val(),
          award: +$(`.award_${_idx}`).val(),
          award_id: award_id
      ...
      for (let i = 0; i < $('#score tbody tr.list').length; i++) {
        ...
        let _item = {
          name: $(`.pname_${_idx}`).val(),
          troublerate: 0, //$(`.trouble_${_idx}`).val(),
          durationrate: 0, // $(`.duration_${_idx}`).val(),
          responserate: 0, // +$(`.response_${_idx}`).val(),
          ratio: 0, // +$(`.rate_${_idx}`).val(),
          percent: 0, // +$(`.percent_${_idx}`).val(),
          amount: +$(`.amount_${_idx}`).val(),
          saleamount: 0, // +$(`.saleamount_${_idx}`).val() || 0,
          poolamount: 0, // +$(`.poolamount_${_idx}`).val() || 0,
          award_id: award_id
        }
        addprocesspersonArr.push(_item)
      }
      ...
      let params_in = {
        name: '',
        amount: 0, // +$('#poolmoney').val(),
        totalmoney: 0, // +$('#totalpool').val() + +$('#poolmoney').val(),
        type: 2,
        award_id: award_id,
      }
      ...
      let params_out = {
        name: '',
        amount: 0, // +$('#poolaward').val(),
        totalmoney: 0, // +$('#totalpool').val() + +$('#poolmoney').val() - +$('#poolaward').val(),
        type: 1,
        award_id: award_id,
      }
      ...
      allpromise.then((r)=>{
        if (isEdit) {
          location.href=`./outreward.html?params=${award_id}`
        } else {
          // 重置
          location.href='./outreward.html'
        }
